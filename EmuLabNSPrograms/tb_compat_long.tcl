...
namespace eval TBCOMPAT {
    ...
    variable FSDIR_PROJ "/q/proj"
    variable PROJROOT	"/proj"

    # ... "/groups",
    variable FSDIR_GROUPS "/q/groups"
...
proc tb-set-hardware {node type args} {
	...
	#return
	#ILIA - this is a hack to automatically add new hardware type
	set hwtypes($type) 1
	set hwtype_class($type) pcvm
	set isremote($type) 0
	set isvirt($type) 1
	set issubnode($type) 0
	set hwtypes($type) 1
	set isremote($type) 0
	set isvirt($type) 1
	set issubnode($type) 0
